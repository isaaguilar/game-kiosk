...
impl Renderer {
    ...
    pub fn draw(&self, buf: &mut Vec<u32>, state: &AppState) {
        ...
        match state {
            ...
            AppState::Playing { current_prompt, .. } => {
                self.draw_playing(buf, current_prompt);
            }
        }
    }
    ...
    fn draw_menu(&self, buf: &mut Vec<u32>, selected: usize) {
        ...
        // Footer hint
        let hint = "Enter/Space start   Esc quit";
        self.draw_text_centered(buf, hint, 18.0, cx, self.height - 24, WHITE);
    }

    // ── Playing ──────────────────────────────────────────────────────────────

    fn draw_playing(&self, buf: &mut Vec<u32>, prompt: &str) {
        let cx = self.width / 2;
        ...
        // Game label top-left
        self.draw_text_centered(buf, "Pictionary", 22.0, 70, 22, WHITE);
