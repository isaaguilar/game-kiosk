...
impl Renderer {
    ...
    pub fn draw(&self, buf: &mut Vec<u32>, state: &AppState) {
        ...
        match state {
            AppState::Loading { status, started_at } => {
            ...
            AppState::Error(err) => {
                ...
                self.draw_multiline_text_centered(buf, err, 24.0, cx, cy, WHITE);
                self.draw_text_centered(buf, "Press Enter to retry   Esc to quit", 18.0, cx, self.height - 24, GRAY);
            }
            AppState::Ready { items, .. } => {
                self.draw_text_centered(buf, "TRIVIA", 72.0, cx, self.height / 4, WHITE);
                let msg = format!("{} questions loaded.", items.len());
                self.draw_text_centered(buf, &msg, 32.0, cx, cy, WHITE);
                self.draw_text_centered(buf, "Press Enter/Space to start", 24.0, cx, cy + 60, WHITE);
            }
            AppState::Question { items, current_idx, start_time, duration } => {
                ...
                self.draw_text_centered(buf, "QUESTION", 26.0, cx, 34, GRAY);
                ...
                self.draw_timer_pie(buf, timer_cx, timer_cy, timer_radius, remaining, timer_color);
                ...
                self.draw_text_centered(buf, "Enter/Space for answer", 18.0, cx, self.height - 24, GRAY);
            }
            AppState::Answer { items, current_idx } => {
                let item = &items[*current_idx];
                self.draw_text_centered(buf, "ANSWER", 26.0, cx, 34, GRAY);
                self.draw_wrapped_text_fit_centered(
                    buf,
                    &item.answer,
                    52.0,
    ...
    fn draw_text_centered(&self, buf: &mut Vec<u32>, text: &str, size: f32, cx: usize, cy: usize, color: u32) {
    ...
    fn draw_multiline_text_centered(&self, buf: &mut Vec<u32>, text: &str, size: f32, cx: usize, cy: usize, color: u32) {
    ...
    fn draw_wrapped_text_fit_centered(
        ...
        self.draw_multiline_text_with_max(
            buf,
            text,
            chosen,
            cx,
